/* ripdraw.c  Version 1.0T 8/1/14  Ripdraw C library for tutorial
 *
 * ripdraw.c contains all the source code for the core Ripdraw library calls except the calls to port drivers
 *
 *
 * The souce code for these calls are external to this file and should be in the corresponding port driver
 *
...
int rd_buffer_check_and_allocate(RD_INTERFACE_BUFFER* buffer, int required_capacity)
{
	...
	tmp = (RD_BYTE*)malloc(required_capacity + buffer->capacity);
    if (!tmp)
    {
...
int Rd_EventMessage(RD_INTERFACE* rd_interface, RD_EVENT** event, RD_UWORD* count)
{
    ...
    return rd_cmd_response_check_and_get_event_data(rd_interface, event, 10, count);
}
